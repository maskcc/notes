...
- 类选择器,__以英文圆点开头,可以任意起名__,来定义内容样式,使用时用`class=name`来选择.
...
###块状元素
css中块级元素都从新的一行开始,并且气候的元素也得另起一行,如`h1`,元素的高度,宽度,,行高以及顶和底边距都可设置.设置`display:block`将元素设置为块级元素.如设置:
...
- 流动模型(Flow)是默认的网页布局模式.
	1. 块状元素都会在所处的包含元素内自上而下按顺序垂直延伸分布,默认状态下,块状元素的宽度在没设置width时都为百分百,实际上,块状元素都会以行的形式占据位置整个行,类似如下(II)那样一直沿伸到行尾

			类似这行,blocking 流动模型(II)
...
- 相对定位,需要设置`position:relative`,通过left,right,top,bottom属性确定元素在正常文档流中的偏移位置.元素会相对于以前的位置移动.移动前的位置保留不动.占据的仍然是以前的位置. 简单来说就是类似以前位置是一个占据空间的框， 将物体相对于那个框进行偏移，但是定位还是在框位置. 
